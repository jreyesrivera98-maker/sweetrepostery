...
export const MoldScaler: React.FC<MoldScalerProps> = ({ recipe }) => {
  ...
  return (
    <div className="ai-panel glass-card bg-[#FDFDFD] rounded-2xl p-6 border border-[#E8E3FF] shadow-sm">
      <h3 className="font-bold text-[#2D3436] mb-4 flex items-center">
        <Calculator size={18} className="mr-2 text-[#6C5CE7]" />
      ...
      <div className="space-y-4 mb-4">
        {recipe.mold_type === 'circular' && (
          <div>
            <label className="block text-sm text-[#636E72] mb-1">Nuevo Diámetro (cm)</label>
            <input 
              type="number" 
              value={targetVal1} 
        ...
        {recipe.mold_type === 'rectangular' && (
          <div className="flex space-x-2">
            <div>
              <label className="block text-sm text-[#636E72] mb-1">Nuevo Ancho</label>
            ...
            <div>
              <label className="block text-sm text-[#636E72] mb-1">Nuevo Largo</label>
        ...
        {recipe.mold_type === 'na' && (
          <div>
            <label className="block text-sm text-[#636E72] mb-1">Nuevas Porciones</label>
            <input type="number" value={targetVal1} onChange={e => setTargetVal1(Number(e.target.value))} className="input-marea w-full p-2 border rounded-lg" />
          </div>
        )}
        
        <div className="bg-[#F4F3FF] p-3 rounded-lg">
          <p className="text-sm font-semibold text-[#6C5CE7]">Factor de escala: {scaleFactor.toFixed(2)}x</p>
        </div>
      </div>
    </div>
  );
